...
//             // Create Logic
//           //  Event: 2026-08-15 10:00
//           // Reminder: 2026-08-15 09:00 ✅
//           // Event: 2026-08-15 10:00
//           // Reminder: 2026-08-15 10:00 ✅
//           // Event: 2026-08-15 10:00
//           // Reminder: 2026-08-15 11:00 ❌
...
class _AddEventScreenState extends State<AddEventScreen> {
  ...
  void _initializeFields() {
    if (_isEditing) {
      ...
      try {
        if (event.reminderTimer != null && event.reminderTimer!.isNotEmpty) {
          _reminderTime = _parseTimeOfDay(event.reminderTimer!);
        } else {
  ...
  /// Automatically clamps reminder date/time if event date/time is moved before reminder
  void _clampReminderToEventLimit() {
  ...
  /// Robust TimeOfDay parser supporting both 24-hour ("09:59:00") and 12-hour ("09:59 AM") formats
  TimeOfDay _parseTimeOfDay(String timeStr) {
    try {
      final trimmed = timeStr.trim();
      final periods = trimmed.split(" ");
      final timeParts = periods[0].split(":");
            ...
            itemBuilder: (context, index) {
              ...
              return GestureDetector(
                ...
                child: Container(
                  padding: const EdgeInsets.all(12),
                  decoration: BoxDecoration(
                    color: isSelected ? eventColors[selectedColorIndex].withValues(alpha: 0.15) : (Theme.of(context).brightness == Brightness.dark ? Theme.of(context).cardColor : AppColor().white),
                    borderRadius: BorderRadius.circular(14),
                    border: Border.all(
                      color: isSelected ? eventColors[selectedColorIndex] : (Theme.of(context).brightness == Brightness.dark ? const Color(0xff3A3A3C) : const Color(0xffE8E8EE)),
                      width: 2,
