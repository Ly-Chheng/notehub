...
import 'package:hive_flutter/hive_flutter.dart';
import 'package:get/get.dart';
import 'package:project_structure/controllers/notes/recently_deleted_controller.dart';
...
class RecentlyDeletedScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: ValueListenableBuilder(
        valueListenable: Hive.box('recently_deleted').listenable(),
        builder: (context, Box box, _) {
          if (box.isEmpty) {
            return const Center(
            ...
            itemBuilder: (context, index) {
              ...
              return Card(
                child: ListTile(
                  ...
                  subtitle: Text("Deleted on: ${note['deletedAt']?.split('T')[0] ?? 'Unknown'}"),
                  trailing: PopupMenuButton<String>(
                    onSelected: (value) {
                      if (value == 'restore') {
                        controller.restoreNote(index);
                      } else {
                        controller.deletePermanently(index);
                      }
                    },
                    itemBuilder: (context) => [
                      const PopupMenuItem(
                        value: 'restore',
                        child: Row(children: [Icon(Icons.restore), SizedBox(width: 8), Text("Restore")]),
                      ),
                      const PopupMenuItem(
                        value: 'delete',
                        child: Row(children: [Icon(Icons.delete_forever, color: Colors.red), SizedBox(width: 8), Text("Delete Forever")]),
                      ),
                    ],
                  ),
                ),
  ...
  void _showDeleteAllDialog(BuildContext context, RecentlyDeletedController controller) {
    Get.defaultDialog(
      title: "Empty Trash?",
      middleText: "All notes in the trash will be permanently deleted.",
